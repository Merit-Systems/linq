...
- **Warmth + price:** call `POST /api/messages/warmth` before telling the user cold/warm or send cost (required — returns `quoted_price_usd` and line-wide warmth, not per wallet).
- **Schema probe:** `check_endpoint_schema` confirms request shape only — not send cost. Do not cite discovery `maxPrice` as the quote.
